...
function Bars({
  ...
  // linear scale one month of 89 files flattens eight years into a rule:
...
export function ClockPlates({ catalog, width }: { catalog: Catalog; width: number }) {
  const coverageW = Math.max(320, Math.round(width * 0.62));
  const intakeW = Math.max(180, width - coverageW - 24);
  ...
  const everyN = Math.max(1, Math.round(catalog.coverage.length / 8));
  return (
    <div className="clock-plates">
      <figure className="clock-figure">
        ...
        <Bars buckets={catalog.coverage} width={coverageW} markIndex={catalog.registerOpenedIndex} labelEvery={everyN} />
      ...
      <figure className="clock-figure">
        ...
        <Bars buckets={catalog.intake} width={intakeW} labelEvery={1} />
        <p className="clock-note">
