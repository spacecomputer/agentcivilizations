...
export function HomeFeed({
  ...
  initialEarlierDay: string | null;
}) {
  // The day address is read off location.search in an effect rather than
  // with useSearchParams. That hook suspends during prerender and opts its
  // whole subtree out of the static HTML — which is why this page used to
  // serve 22 characters to every crawler and every share unfurler, prose
  // included. Reading it after mount costs one extra fetch on the paginated
  // views only, and hands the register its front page back.
  const [until, setUntil] = useState<string | null>(null);
  useEffect(() => {
    const raw = new URLSearchParams(window.location.search).get("until");
    // /?until=zzz used to throw on toISOString and land in the catch below,
    // which then reported an empty register.
    setUntil(raw && /^\d{4}-\d{2}-\d{2}$/.test(raw) ? raw : null);
  }, []);
  const [page, setPage] = useState<FeedPage | null>(
    initialGroups.length
      ? { groups: initialGroups, earlierDay: initialEarlierDay }
      : null,
  );
  ...
  useEffect(() => {
    // Skip the first pass when the build already handed us this exact page.
    if (!hydrated && !until && initialGroups.length) {
      setHydrated(true);
      return;
    }
    let cancelled = false;
    ...
    fetchFeedPage(until)
      ...
      .catch(() => {
        if (cancelled) return;
        setFailed(true);
        setPage({ groups: [], earlierDay: null });
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <>
      ...
      {failed && (
        <p className="dim">
          The register could not be read just now. Nothing has been lost — the
          record is intact and reachable at{" "}
          <a href="/api/figures.json">/api/figures.json</a>. Try again shortly.
        </p>
      )}
      {!failed && page !== null && page.groups.length === 0 && (
